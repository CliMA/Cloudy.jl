...
function main()
  ...
  # gamma IC this time
  dist_init = x-> N*x^(k_shape-1)*exp(-x/theta_scale)/(theta_scale^k_shape)/gamma(k_shape)
  ...
  ############################### PLOTTING ####################################
    ...
    savefig("rbf/Comparison/golovin_badIC/RBF_golovin_badIC_gaussiandist10.png")
  ...
  t-> (moments_init[1]*exp(-b*moments_init[2]*t))/moments_init[1],
      linewidth=3,
      title="\$C(m, m') = k\$ (Smolu. 1916) vs. GP-Collocation with 10 RBF",
      xaxis="time",
      yaxis="M\$_k\$(time)",
      xlims=tspan,
      ylims=(0, 10), 
  ...
  savefig("rbf/Comparison/golovin_badIC/RBF_golovin_badIC_gaussian10.png")
