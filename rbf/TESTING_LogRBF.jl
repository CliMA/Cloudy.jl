...
function main()
  ...
  Nb = 3
  rbf_mu = select_rbf_locs(15.0, 100.0, 3)
  rbf_sigma = select_rbf_shapes(rbf_mu, smoothing_factor=1.6)
  basis = Array{PrimitiveUnivariateBasisFunc}(undef, Nb)
  for i = 1:Nb
    basis[i] = GaussianBasisFunction(5/3*log(rbf_mu[i])-2/3*log(rbf_sigma[i]), sqrt(-1/sqrt(3)*log(rbf_sigma[i]^2/rbf_mu[i]^2)))
  end
  println(basis)

  # Precompute the various matrices
  zstart=log(1e-12*rbf_mu[1])
  zstop=log(1e12*rbf_mu[end])
  A = get_rbf_inner_products(basis)
  Source = get_kernel_rbf_source(basis, rbf_mu, kernel_func, zstart=zstart)
  Sink = get_kernel_rbf_sink(basis, rbf_mu, kernel_func, zstart=zstart, zstop=zstop)
  mass_cons = get_mass_cons_term(basis, zstart=zstart, zstop=zstop)
  ...
  ############################### PLOTTING ####################################
    ...
    x = exp.(z)
    plot(x, 
      evaluate_rbf(basis, c0, z)/sum(c0),
      linewidth=2,
    ...
    plot!(x,
      evaluate_rbf(basis, cj, z)/sum(cj),
      linewidth=2,
      label="t = 1"
    )
