    ...
    for j=1:Nb
        for i=1:Nb
            for (k, moment_order) in enumerate(moment_list)
                Φ[(k-1)*Nb+j,i] = rbf_locs[j]^(moment_order-1) * evaluate_rbf(basis[i], rbf_locs[j])
            end
        end
    end
...
function get_kernel_rbf_sink(basis::Array{PrimitiveUnivariateBasisFunc, 1}, rbf_locs::Array{FT}, moment_list::Array{FT, 1}, kernel::Function; xstart::FT = eps(), xstop::FT = 1e6) where {FT <: Real}
    ...
    for i=1:Nb
        for j=1:Nb
            for k=1:Nb
                integrand = y -> basis_func(basis[k])(y)/y*kernel([rbf_locs[i], y])
    ...
    for i=1:Nb
        for (l, moment_order) in enumerate(moment_list)
            N[(l-1)*Nb+i,:,:] = N[i,:,:] .* rbf_locs[i]^(moment_order-1)
        end
    end
...
function get_kernel_rbf_source(basis::Array{PrimitiveUnivariateBasisFunc, 1}, rbf_locs::Array{FT}, moment_list::Array{FT, 1}, kernel::Function; xstart::FT = eps()) where {FT <: Real}
    ...
    for i=1:Nb
        for j=1:Nb
            for k=1:Nb
                integrand = y-> basis_func(basis[j])(rbf_locs[i] - y)/(rbf_locs[i] - y)*basis_func(basis[k])(y)/y* kernel([rbf_locs[i] - y, y])
                M[i, j, k] = quadgk(integrand, xstart, rbf_locs[i])[1]*rbf_locs[i]
            end
        end
    end
    ...
    for i=1:Nb
        for (l, moment_order) in enumerate(moment_list)
            M[(l-1)*Nb+i,:,:] = M[i,:,:] .* rbf_locs[i]^(moment_order-1)
        end
    end
