...
"""
abstract type PDistribution{FT} end
...
"""
abstract type Primitive{FT} <: PDistribution{FT} end
...
"""
  Mixture{FT} <: PDistribution{FT}

A particle mass distribution function that is a mixture of
primitive or truncated subdistribution functions.

# Constructors
  Mixture(dists::PDistribution{Real}...)
  Mixture(dist_arr::Array{PDistribution{FT}})

# Fields
$(DocStringExtensions.FIELDS)
"""
struct Mixture{FT} <: PDistribution{FT}
  "array of distributions"
  subdists::Array{PDistribution{FT}}

  function Mixture(dists::PDistribution{FT}...) where {FT<:Real}
    if length(dists) < 2
...
function Mixture(dist_arr::Array{PDistribution{FT}}) where {FT<:Real}
  Mixture(dist_arr...)
end
...
"""
function moment(dist::PDistribution{FT}, q::FT) where {FT<:Real}
...
"""
function density(dist::PDistribution{FT}, x::FT) where {FT<:Real}
  if any(x .< zero(x))
...
function update_params(dist::Mixture{FT}, values::Array{FT}) where {FT<:Real}
  ...
  i = 1
  dist_arr = Array{PDistribution{FT}}([])
...
function update_params_from_moments(dist::PDistribution{FT}, m::Array{FT}) where {FT<:Real}
