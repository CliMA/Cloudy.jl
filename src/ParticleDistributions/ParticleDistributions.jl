...
"""
function partial_moment_func(
    dist::ExponentialPrimitiveParticleDistribution{FT},
) where {FT<:Real}
    # moment_of_dist = n * θ^q * Γ(q+1)
    function f(q, x_threshold)
        dist.n * dist.θ^q * gamma_inc(q + FT(1), x_threshold / dist.θ)[1] * gamma(q + FT(1))
    end
    return f
end
...
function partial_moment_func(dist::GammaPrimitiveParticleDistribution{FT}) where {FT<:Real}
    ...
    function f(q, x_threshold)
        dist.n *
        dist.θ^q *
        gamma_inc(q + dist.k, x_threshold / dist.θ)[1] *
        gamma(q + dist.k) / gamma(dist.k)
    end
    return f
end
...
"""
function partial_moment(
    dist::AbstractParticleDistribution{FT},
    q::FT,
    x_threshold::FT,
) where {FT<:Real}
...
function get_moments(pdist::GammaPrimitiveParticleDistribution{FT}) where {FT<:Real}
    return [
        pdist.n,
        pdist.n * pdist.k * pdist.θ,
        pdist.n * pdist.k * (pdist.k + 1) * pdist.θ^2,
    ]
end
...
function get_moments(pdist::LognormalPrimitiveParticleDistribution{FT}) where {FT<:Real}
    return [
        pdist.n,
        pdist.n * exp(pdist.μ + pdist.σ^2 / 2),
...
function density_func(dist::GammaPrimitiveParticleDistribution{FT}) where {FT<:Real}
    ...
    function f(x)
        dist.n .* x .^ (dist.k .- 1) ./ dist.θ .^ dist.k ./ gamma.(dist.k) .*
        exp.(-x ./ dist.θ)
    end
    return f
end
...
function density_func(dist::LognormalPrimitiveParticleDistribution{FT}) where {FT<:Real}
    ...
    function f(x)
        dist.n .* exp.(-((log.(x) - dist.μ) .^ 2 ./ (2 * dist.σ^2))) ./
        (x .* dist.σ .* sqrt(2 * π))
    end
    return f
end
...
function update_dist_from_moments(
    ...
    if moments[1] > eps(FT) && moments[2] > eps(FT) && moments[3] > eps(FT)
        μ = max(
            ...
            min(
                param_range.μ[2],
                log(moments[2]^2 / moments[1]^(3 / 2) / moments[3]^(1 / 2)),
            ),
        )
        σ = max(
            param_range.σ[1],
            min(param_range.σ[2], sqrt(log(moments[1] * moments[3] / moments[2]^2))),
        )
...
function moment_source_helper(
    ...
    y_func(j) =
        j <= n_bins ? exp(logx(x_min, j, dx)) * f(exp(logx(x_min, j, dx))) :
        zero(typeof(dx))
...
function moment_source_helper(
    ...
    f(x) =
        x^(p1 + k - 1) * exp(-x / θ) * gamma_inc(p2 + k, (x_threshold - x) / θ)[1] * γ_p2k
...
function get_standard_N_liq(
    ...
    return mapreduce(
        j -> partial_moment(pdists[j], FT(0), size_cutoff),
        +,
        ntuple(identity, N),
    )
end
...
function get_standard_M_liq(
    ...
    return mapreduce(
        j -> partial_moment(pdists[j], FT(1), size_cutoff),
        +,
        ntuple(identity, N),
    )
end
