...
dist_init = [
    ...
    GammaPrimitiveParticleDistribution(FT(0), FT(1e-8), FT(1)),
    GammaPrimitiveParticleDistribution(FT(0), FT(1e-6), FT(1)),
    GammaPrimitiveParticleDistribution(FT(0), FT(1e-4), FT(1)),
]
...
coal_data = initialize_coalescence_data(
    ...
    NProgMoms,
    norms = norms,
    dist_thresholds = [FT(1e-9), FT(1e-7), FT(1e-5), Inf],
)
