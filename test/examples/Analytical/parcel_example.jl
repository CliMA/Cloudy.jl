...
import Cloudy: rflatten, get_dist_moments_ind_range
...
function parcel_model_cloudy(dY, Y, p, t)
    ...
    (; wps, tps, aps, w, pdists, NProgMoms) = p
    # Y values
    Sₗ = Y[1]
    p_air = Y[2]
    T = Y[3]
    qᵥ = Y[4]
    moments = tuple(Y[5:end]...)
...
function run_parcel_cloudy(Yinit, t_0, t_end, pp)
    FT = typeof(t_0)

    println(" ")
    println("Size distributions: ", pp.pdists)
...
function init_conditions(ρₗ, type::String)
    ...
    moments_init = rflatten(ntuple(length(dist_init)) do k
        tuple(CPD.get_moments(dist_init[k])...)
    end)
    ml_v = moments_init[2]
    if length(dist_init) > 1
        ml_v += moments_init[4]
    end
    return (dist_init, NProgMoms, moments_init, ml_v)
end

function get_spectrum(pdists, NProgMoms, moments)
