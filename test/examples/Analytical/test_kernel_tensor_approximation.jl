...
p1 = contourf(x, y, z_kernel, xaxis = "x", yaxis = "y", title = "Kernel Function", colorbar_exponentformat = "power", clim=(0, 2.5e-6))
p2 = contourf(x, y, z_tensor, xaxis = "x", yaxis = "y", title = "Kernel Tensor", colorbar_exponentformat = "power", clim=(0, 2.5e-6))
...
#     for j in 1:n
#         z_kernel[i, j] = i < j ? NaN : kernel_func(x[i], y[j])
#         z_tensor[i, j] = i < j ? NaN : compute_kernel_from_tensor_matrix(matrix_of_kernels, (FT(0), FT(5e-10), FT(1e-6)), x[i], y[j])
#     end
# end

# p1 = contourf(x, y, z_kernel, xaxis = "x", yaxis = "y", title = "Kernel Function", colorbar_exponentformat = "power", clim=(0, 1e-8))
# p2 = contourf(x, y, z_tensor, xaxis = "x", yaxis = "y", title = "Kernel Tensor", colorbar_exponentformat = "power", clim=(0, 1e-8))
