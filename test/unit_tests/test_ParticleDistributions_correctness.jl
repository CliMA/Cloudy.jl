...
@test moment_source_helper(dist, 0.0, 0.0, 0.5, Val(100)) ≈ 2.642e-1 rtol = rtol
@test moment_source_helper(dist, 1.0, 0.0, 0.5, Val(100)) ≈ 4.015e-2 rtol = rtol
@test moment_source_helper(dist, 1.0, 1.0, 0.5, Val(100)) ≈ 4.748e-3 rtol = rtol
dist = GammaPrimitiveParticleDistribution(1.0, 0.5, 2.0)
@test moment_source_helper(dist, 0.0, 0.0, 0.5, Val(100)) ≈ 1.899e-2 rtol = rtol
@test moment_source_helper(dist, 1.0, 0.0, 0.5, Val(100)) ≈ 3.662e-3 rtol = rtol
@test moment_source_helper(dist, 1.0, 1.0, 0.5, Val(100)) ≈ 5.940e-4 rtol = rtol
...
Npt = 91
x = SVector{Npt}(collect(range(1.0, 10.0, Npt)))
y = x .^ 2
